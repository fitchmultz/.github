...
const focused='compiles experimental source lookups|compiles experimental network source lookups|makes close --all exclusive|warns after record start when ffmpeg is missing|timed-out raw recording batch|main browser command never starts|registered upgrade output keeps failure precedence: (timeout|abort)';
...
try {
 ...
 if(process.platform==='win32') {
  const current=Object.fromEntries(manifest.parentFiles.map(f=>[f,readFileSync(join(dev,f))]));
  try {for(const f of manifest.parentFiles)writeFileSync(join(dev,f),readFileSync(join(here,'before',basename(f))));
   const old=check('old-nine',[...testArgs,'--test-name-pattern='+focused,...manifest.parentFiles]);report.oldControl={status:old.status,failures:(readFileSync(join(out,'old-nine.stdout.log'),'utf8').match(/^not ok /gm)??[]).length};assert.equal(old.status,1,'unchanged old fixtures must remain red');
  } finally {for(const [f,bytes] of Object.entries(current))writeFileSync(join(dev,f),bytes);}
 }
 check('focused',[...testArgs,'--test-name-pattern='+focused,...manifest.parentFiles]);check('full-five',[...testArgs,...manifest.parentFiles]);
 assert.deepEqual(hashes(),manifest.files);report.finalHashes=hashes();assert.ok(Object.entries(report.runs).filter(([n])=>n!=='old-nine').every(([,r])=>r.status===0),'all corrected checks must pass');report.result='passed';
