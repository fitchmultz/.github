...
The fork is built once per workflow and shared as an artifact. Its cache includes runner architecture, Node version, fork commit, automation revision, lockfile and hydrated model-data contents. Reverse-dependency CI can supply the fork's existing source-snapshot artifact from the same run, so all gates use that frozen model data. Other public callers build directly from public source without cross-repository artifact credentials.
...
The previous official target is a diagnostic comparison when it differs from a candidate; it is not a blanket promise of historical support. Node/OS lanes are selected by the fleet and existing repository-specific workflows. Additional platform lanes use Node 24; the primary platform also tests the declared Node floor. Routine checks use empty profiles and no provider credentials. These results do not certify live browser, Cursor, Oracle, OAuth or desktop operations. Those existing release gates remain separate when a shipped change touches their behavior. An unavailable platform must be recorded as unqualified or explicitly waived, never counted as passed.
