...
Windows isolation retains the native `PSModulePath` alongside Windows runtime variables so PowerShell can discover OS cmdlets normally; HOME, configuration and temp roots remain private, without inherited credentials. The required `windows-environment` self-check runs a real `Get-Process` identity query under that isolation via bash on Windows. It warms ambient PowerShell once to separate cold CLR startup from the environment regression, then enforces the consumer's 5-second query bound. The `automation` aggregate requires this native job; a non-Windows skip cannot qualify it.
