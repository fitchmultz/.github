...
Package contract commands have a 10-minute watchdog by default. The fleet inventory gives Subagents' Linux/macOS contract commands 20 minutes for the full native integration suite and package smoke, and Browser's Windows contract command 15 minutes for its full serial suite and package smoke. That Windows job has a 35-minute limit for cold installation and the remaining consumer checks; other jobs retain 25 minutes. These are whole-command and whole-job CI budgets; individual tests and production deadlines remain unchanged. Each qualification receipt records the selected command budget.
