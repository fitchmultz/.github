...
const selected = requested === "all" ? fleet : fleet.filter((entry) => `fitchmultz/${entry.repo}` === requested);
assert.ok(selected.length, `Unknown fleet repository: ${requested}`);
...
const needsHost = selected.some((entry) => entry.kind !== "cli");
const override = needsHost && process.env.OFFICIAL_VERSION ? await release(process.env.OFFICIAL_VERSION) : undefined;
const baseline = needsHost ? await release(targets.official) : undefined;
const include = [];
for (const entry of selected) {
  ...
  const hosts = entry.kind === "cli" ? [{ host: "none", label: "cli", version: "" }] : [
    { host: "official", label: "official", version: candidate },
    { host: "fork", label: "fork", version: "" },
    ...(candidate === baseline ? [] : [{ host: "official", label: "baseline", version: baseline }]),
  ];
  const platforms = [
