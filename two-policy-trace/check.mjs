// Fresh native original five-file prefix with callback/lock observation only.
...
try {
  ...
  const testEnv = { ...env, POLICY_TRACE_FILE: join(out, 'lock-trace'), PI_COMPAT_HOST: 'official', PI_COMPAT_EXPECTED_VERSION: host.version,
  ...
  for (const file of ['extensions/agent-browser/lib/process-identity.ts', 'extensions/agent-browser/lib/managed-session-policy-lock.ts']) originals[file] = readFileSync(join(dev, file), 'utf8');
  ...
  const policy = await check('policy-trace', 'npm', ['exec', '--', 'tsx', '--test', '--test-reporter=tap', '--test-concurrency=1', 'test/agent-browser.managed-session-policy-lock.test.ts'], testEnv);
  report.postRunInstrumentedHashes = hashes(); assert.deepEqual(report.postRunInstrumentedHashes, expectedInstrumented);
  report.result = result.status === 0 && !result.timedOut && policy.status === 0 && !policy.timedOut ? 'passed' : 'failed';
